...
def get_credentials():
    ...
    st.error("Google credentials not found. Add credentials in Streamlit Cloud secrets.")
    st.stop()
...
def load_products():
    sheet = client.open(SPREADSHEET_NAME).worksheet("Products")
...
def load_terms():
    try:
        sheet = client.open(SPREADSHEET_NAME).worksheet("Terms")
        data = sheet.get_all_values()
...
def load_solution_templates():
    try:
        ...
        df["Template Name"] = df["Template Name"].astype(str).str.strip()
        df["Identification"] = df["Identification"].astype(str).str.strip()
        df["Short Name"] = df["Short Name"].astype(str).str.strip()
        df["Locked"] = df["Locked"].astype(str).str.strip()

        df["Line No"] = pd.to_numeric(df["Line No"], errors="coerce").fillna(0)
        df["Qty"] = pd.to_numeric(df["Qty"], errors="coerce").fillna(1)
        df["Discount"] = pd.to_numeric(df["Discount"], errors="coerce").fillna(0)

        df = df.sort_values(["Template Name", "Line No"])
